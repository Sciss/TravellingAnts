...
/**
  * Runs the solver on a particular graph for exemplary purposes.
  */
object App {
  type Node = Int

  val ant   : Ant   [Node]  = new Ant
  val colony: Colony[Node]  = new Colony(ant)

  /**
    * The scale factor is used to scale the location coordinates of points to
    * produce a reasonably-sized image.
    */
  def nodeAttr(scaleFactor: Double, lookup: Node => Point)(node: Node): Map[String, String] = {
    val p = lookup(node)
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    implicit val exec: ExecutionContext = ExecutionContext.fromExecutorService(es)

    val t0        = System.currentTimeMillis()
    val fut       = colony.solve(graph, iterations = 10000, listener = loggingListener(log))
